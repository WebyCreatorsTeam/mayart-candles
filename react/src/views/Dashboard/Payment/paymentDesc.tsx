...
const AboutDesc: FC<IPaymentDesc> = ({ id, desc }) => {
    ...
    const handleChangeTitle = async () => {
        try {
            ...
            const { data: { continueWork, message } } = await axios.patch(`http://localhost:7575/payment/update-payment-desc?token=${token}`, { id, paymentDesc })
            if (continueWork) {
                alert(message)
    ...
    return (
        ...
        {!editDesc && (
            <>
             {/*  <p className="aboutDash__text">{paymentDesc}</p>*/} 
        ...
        {editDesc && (
            <>
                <textarea
                    ...
                    onChange={(ev: any) => setPaymentDesc(ev.target.value)}></textarea>
                <button
