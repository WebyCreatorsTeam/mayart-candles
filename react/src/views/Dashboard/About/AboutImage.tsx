...
const AboutImage: FC<IAboutImage> = ({ id, img, idx }) => {
  const [aboutImage, setAboutImage] = useState<{ _id: string; img: string }>(
    img,
  );
  ...
  const handleUpload = async () => {
    try {
      ...
      const res = await axios.patch(
        `http://localhost:7575/about/update-about-image?token=${token}&id=${id}&oldURL=${img.img}&oldId=${img._id}`,
        data,
        {
          headers: {
    ...
    <>
      <div className="aboutDash__images--imageItem">
        ...
        <div className="aboutDash__images--imageItem__btnSection">
          <button onClick={() => setPopUpEditImage(true)}>
        ...
        <PopUp>
          ...
          <div className="popupEditImage">
            <p className="popupEditImage--text">
              נא לבחור תמונה ברוחב 530px וגובה 700px
            </p>
            <div
              className={
                prevFileShow
                  ? "popupEditImage--uploadBtn"
                  : "popupEditImage--uploadBtn--grided"
              }
            >
              <UploadFile
                loader={loader}
            ...
            </div>
            {
              prevFileShow && (
                ...
                <img
                  src={prevFileShow}
                  alt="project"
                  className="edit_project__image2"
                />
              )
              // </div>
            }
