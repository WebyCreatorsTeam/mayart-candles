...
const ColorEdit: FC<IColorEdit> = ({ setPopUpEditColors, setCandleColors }) => {
  ...
  const [newColor, setNewColor] = useState<{ color: string; hexCode: string }>({
    color: "",
    hexCode: "",
  });
  ...
  const hendleFillInput = (ev: any) => {
    setNewColor((color: any) => {
      return { ...color, [ev.target.name]: ev.target.value };
    });
  };
  ...
  const handleAddColor = async () => {
    try {
      ...
      const {
        data: { continueWork, message, colors },
      } = await axios.post(`${BASE_API}/candles/add-color?token=${token}`, {
        id,
        newColor,
      });
      if (continueWork) {
