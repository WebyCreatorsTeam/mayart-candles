...
const FragEdit: FC<IFragEdit> = ({
  ...
  const handleAddFrag = async () => {
    try {
      ...
      const {
        data: { continueWork, message, fragrances },
      } = await axios.post(`${BASE_API}/candles/add-frag?token=${token}`, {
        id,
        newFrag,
      });
      if (continueWork) {
  ...
  return (
    <PopUp>
      ...
      <input
        ...
        onChange={(ev: any) => setNewFag(ev.target.value)}
      />
      <button
