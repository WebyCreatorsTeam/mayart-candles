...
const PriceEdit: FC<IPriceEdit> = ({
  ...
  const handleChangePrice = async () => {
    try {
      setLoader(true);
      if (Number(candlePrice) === 0 && Number(candleSalePrice) > 0)
        return alert("לא ניתן לתת הנחה במידה והמחיר שווה לאפס");
      if (Number(candlePrice) < Number(candleSalePrice))
        return alert("המחיר הרגיל לא יכול להיות יותר נמוך ממחיר הנחה");
      ...
      const {
        ...
        `${BASE_API}/candles/change-candle-price?token=${token}`,
        { id, price: candlePrice, salePrice: candleSalePrice },
      );
      if (continueWork) {
        alert(message);
  ...
  return (
    <>
      ...
      <input
        ...
        defaultValue={candlePrice}
        onChange={(ev: any) => setCandlePrice(ev.target.value)}
      />
      <label>מחיר הנחה במידה ויש</label>
      <input
        type="number"
        className="updateInput"
        defaultValue={candleSalePrice}
        onChange={(ev: any) => setCandleSalePrice(ev.target.value)}
      />
      <button
        onClick={handleChangePrice}
        className={loader ? "unactiveBtn" : "actionBtn"}
      >
        {loader ? "מעדכן את הפרטים" : "עדכן מחירים"}
      </button>
      <button
        className={loader ? "unactiveBtn" : "actionBtn"}
        onClick={() => {
