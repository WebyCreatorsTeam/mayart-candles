...
const DashcoardCandle = () => {
  const { candle, categories } = useLoaderData() as { candle: ICandles } & {
...
const hendleGetCandle = async (id: string) => {
  const {
    data: { continueWork, candle },
  } = await axios.post(`${BASE_API}/candles/get-one-candle`, { id });
...
const handleGetCategories = async () => {
  const {
    data: { continueWork, categories },
  } = await axios.get(`${BASE_API}/categories/get-categories`);
