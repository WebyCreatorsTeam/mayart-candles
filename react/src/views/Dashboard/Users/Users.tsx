    ...
    <Suspense fallback={<h1 className="no_data_text">Loading...</h1>}>
      <Await resolve={admins}>
        <section className="user">
              ...
              <div className="gridUserName">
                <div className="gridName" key={idx}>
                  <p className="iconName">
                    {adm.name.slice(0, 2).toLocaleUpperCase()}
                  </p>{" "}
                  {/* בעיצוב של עמוד יוזרים, האותיות העיגול */}
                  <p>{adm.name}</p> {/* שם של משתמש */}
                  {/* {adm.role && (<button>מחיקת משתמש</button>) } */}{" "}
                  {/* Need to add in the model role of user */}
                </div>
          ...
          <Link className="link" to="/dashboard/regist">
            {" "}
            הוספת משתמש
          </Link>
        </section>
...
const hendleGetAdmins = async () => {
  ...
  const { data } = await axios.get(
    `https://mayart-candles-api.vercel.app/admin/all-admins?token=${token}`,
  );
  console.log(data);
